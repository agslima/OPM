...
# Prep packages
sudo apt-get install -y libtool openssl libxml2-dev gcc libboost-dev make flex bison byacc groff libssl-dev >> /tmp/azuredeploy.log.$$ 2>&1
...
sudo cp -rf torque-package-* /var/www/deb/amd64/
sudo cp -rf contrib/init.d/pbs_mom /var/www/deb/amd64/pbs_mom
sudo cp -rf contrib/init.d/pbs_server /var/www/deb/amd64/pbs_server
sudo cp -rf contrib/init.d/trqauthd /var/www/deb/amd64/trqauthd
apt-get install --assume-yes apache2
sudo ln -sf /var/www/deb /var/www/html/deb

for service_name in trqauthd pbs_server pbs_sched
do
sudo update-rc.d -f $service_name remove
sudo cp -rf contrib/init.d/debian.$service_name /etc/init.d/$service_name >> /tmp/azuredeploy.log.$$ 2>&1
sudo update-rc.d $service_name defaults >> /tmp/azuredeploy.log.$$ 2>&1
...
sudo service pbs_sched start >> /tmp/azuredeploy.log.$$ 2>&1
sudo service pbs_server start >> /tmp/azuredeploy.log.$$ 2>&1
sudo service --status-all >> /tmp/azuredeploy.log.$$ 2>&1
